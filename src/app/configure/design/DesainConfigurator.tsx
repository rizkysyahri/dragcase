...
import {
  ...
  MATERIALS,
  MODELS,
} from "@/validators/option-selector";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const DesainConfigurator: React.FC<DesainConfiguratorProps> = ({
  configId,
  imageUrl,
  imageDimensions,
}) => {
                      ...
                      <div>
                        {selectTableOptions.map((option) => (
                          <RadioGroup.Option
                            ...
                            className={({ active, checked }) =>
                              cn(
                                "relative block cursor-pointer rounded-lg bg-white px-6 py-4 shadow-sm border-2 border-zinc-200 focus:ring-0 outline-none sm:flex sm:justify-between",
                                {
...
export default DesainConfigurator;
